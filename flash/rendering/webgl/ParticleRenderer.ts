...
{
    private static _particleRender:ParticleRenderer;
    ...
    public render(container:DisplayObject):void
    {
    ...
    public _generateOneMoreBuffer(container:ParticleContainer):ParticleBuffer
    {
    ...
    public uploadVertices(children:DisplayObject[], startIndex:number, amount:number, array:number[], stride:number, offset:number):void
    {
        let w0 = 0;
        let w1 = 0;
        let h0 = 0;
        ...
        {
            const sprite:DisplayObject = children[startIndex + i];
            if(sprite instanceof Bitmap)
            {
    ...
    public uploadPosition(children:DisplayObject[], startIndex:number, amount:number, array:number[], stride:number, offset:number):void
    {
        for (let i = 0; i < amount; i++)
        {
    ...
    public uploadRotation(children:DisplayObject[], startIndex:number, amount:number, array:number[], stride:number, offset:number):void
    {
        for (let i = 0; i < amount; i++)
        {
    ...
    public uploadTint(children:DisplayObject[], startIndex:number, amount:number, array:number[], stride:number, offset:number):void
    {
        for (let i = 0; i < amount; ++i)
        {
            const sprite:DisplayObject = children[startIndex + i];
            ...
            {   
                ...
                const argb = alpha < 1.0 && premultiplied ? Utils.premultiplyTint(sprite._tintRGB, alpha) : sprite._tintRGB + (alpha * 255 << 24);
                array[offset] = argb;
