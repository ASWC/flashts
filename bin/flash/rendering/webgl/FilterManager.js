    ...
    class FilterManager // extends WebGLRenderer
     {
        constructor() {
            ...
            //this.managedFilters = [];
            //this.renderer.addEventListener(Event.RENDER, this.onPrerender, this);
        ...
        pushFilter(target, filters) {
            ...
            if (!filterData) {
                ...
                filterState.sourceFrame = filterState.destinationFrame = this.stageContext.activeRenderTarget.size;
                filterState.renderTarget = this.stageContext.activeRenderTarget;
            ...
            const fullScreen = target.filterArea
                ...
                && target.filterArea.width === this.stageContext.screen.width
                && target.filterArea.height === this.stageContext.screen.height;
            // for now we go off the filter of the first resolution..
            const resolution = filters[0].resolution;
            const padding = filters[0].padding | 0;
            const targetBounds = fullScreen ? this.stageContext.screen : (target.filterArea || target.getBounds(true));
            ...
            // this should stop the strange side effects that can occur when cropping to the edges
            const renderTarget = this.getPotRenderTarget(this.stageContext.context, sourceFrame.width, sourceFrame.height, resolution);
            ...
            this.stageContext.bindRenderTarget(renderTarget);
            renderTarget.clear();
        }
        /**
        ...
        popFilter() {
            ...
            if (filters.length === 1) {
                ...
                let flop = this.getPotRenderTarget(this.stageContext.context, currentState.sourceFrame.width, currentState.sourceFrame.height, currentState.resolution);
        ...
        applyFilter(filter, input, output, clear) {
            const gl = this.stageContext.context;
            let shader = filter.glShaders[this.stageContext.getContextID()];
            // cacheing..
            if (!shader) {
                if (filter.glShaderKey) {
                    shader = this.shaderCache[filter.glShaderKey];
                    if (!shader) {
                        shader = new GLShader_1.GLShader(this.gl, filter.vertexSrc, filter.fragmentSrc);
                        filter.glShaders[this.stageContext.getContextID()] = this.shaderCache[filter.glShaderKey] = shader;
                        this.managedFilters.push(filter);
                    }
                }
                else {
                    shader = filter.glShaders[this.stageContext.getContextID()] = new GLShader_1.GLShader(this.gl, filter.vertexSrc, filter.fragmentSrc);
                    this.managedFilters.push(filter);
                }
            ...
            this.stageContext.bindVao(this.quad.vao);
            this.stageContext.bindRenderTarget(output);
            if (clear) {
                gl.disable(gl.SCISSOR_TEST);
                this.stageContext.clear([1, 1, 1, 1]); // [1, 1, 1, 1]);
                gl.enable(gl.SCISSOR_TEST);
            }
            // in case the render target is being masked using a scissor rect
            if (output === this.stageContext.getMaskManager().scissorRenderTarget) {
                this.stageContext.getMaskManager().pushScissorMask(null, this.stageContext.getMaskManager().scissorData);
            }
            this.stageContext.bindShader(shader);
            ...
            const tex = this.stageContext.emptyTextures[0];
            this.stageContext.boundTextures[0] = tex;
            // this syncs the PixiJS filters  uniforms with glsl uniforms
            this.syncUniforms(shader, filter);
            this.stageContext.getRenderState().setBlendMode(filter.blendMode);
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, input.texture.texture);
            this.quad.vao.draw(this.stageContext.context.TRIANGLES, 6, 0);
            ...
            for (const i in uniformData) {
                ...
                if (type === 'sampler2d' && uniforms[i] !== 0) {
                    if (uniforms[i].baseTexture) {
                        shader.uniforms[i] = this.stageContext.bindTexture(uniforms[i].baseTexture, textureCount);
                    }
                    else {
                        ...
                        this.stageContext.boundTextures[textureCount] = this.stageContext.emptyTextures[textureCount];
                        this.stageContext.context.activeTexture(this.stageContext.context.TEXTURE0 + textureCount);
        ...
        getRenderTarget(clear, resolution) {
            ...
            const renderTarget = this.getPotRenderTarget(this.stageContext.context, currentState.sourceFrame.width, currentState.sourceFrame.height, resolution || currentState.resolution);
        ...
        calculateScreenSpaceMatrix(outputMatrix) {
            const currentState = this.filterData.stack[this.filterData.index];
            return Utils_1.Utils.calculateScreenSpaceMatrix(outputMatrix, currentState.sourceFrame, currentState.renderTarget.size);
        }
        /**
        ...
        calculateNormalizedScreenSpaceMatrix(outputMatrix) {
            const currentState = this.filterData.stack[this.filterData.index];
            return Utils_1.Utils.calculateNormalizedScreenSpaceMatrix(outputMatrix, currentState.sourceFrame, currentState.renderTarget.size, currentState.destinationFrame);
        }
        /**
        ...
        calculateSpriteMatrix(outputMatrix, sprite) {
            const currentState = this.filterData.stack[this.filterData.index];
            return Utils_1.Utils.calculateSpriteMatrix(outputMatrix, currentState.sourceFrame, currentState.renderTarget.size, sprite);
        }
        /**
        ...
        destroy(contextLost = false) {
            ...
            for (let i = 0; i < filters.length; i++) {
                if (!contextLost) {
                    filters[i].glShaders[this.stageContext.getContextID()].destroy();
                }
                delete filters[i].glShaders[this.stageContext.getContextID()];
            }
            this.shaderCache = {};
            if (!contextLost) {
                this.emptyPool();
        ...
        getPotRenderTarget(gl, minWidth, minHeight, resolution) {
            let key = FilterManager.screenKey;
            minWidth *= resolution;
            minHeight *= resolution;
            if (minWidth !== this._screenWidth
                || minHeight !== this._screenHeight) {
                // TODO you could return a bigger texture if there is not one in the pool?
                minWidth = Utils_1.Utils.nextPow2(minWidth);
                minHeight = Utils_1.Utils.nextPow2(minHeight);
            ...
            if (!renderTarget) {
                ...
                const tex = this.stageContext.boundTextures[0];
                gl.activeTexture(gl.TEXTURE0);
         ...
         */
        onPrerender() {
            if (this._screenWidth !== this.stageContext.getCanvasView().width
                || this._screenHeight !== this.stageContext.getCanvasView().height) {
                this._screenWidth = this.stageContext.getCanvasView().width;
                this._screenHeight = this.stageContext.getCanvasView().height;
                const textures = this.pool[FilterManager.screenKey];
                if (textures) {
                    for (let j = 0; j < textures.length; j++) {
                        textures[j].destroy(true);
                    }
                }
                this.pool[FilterManager.screenKey] = [];
            }
        }
    }
    FilterManager.screenKey = 'screen';
