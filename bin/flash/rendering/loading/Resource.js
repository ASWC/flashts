 ...
 */
        ...
        static parseUri(str, mode = null) {
            var o = Resource.options, m = o.parser[o.strictMode ? "strict" : "loose"].exec(str), uri = {}, i = 14;
            while (i--)
                uri[o.key[i]] = m[i] || "";
            uri[o.q.name] = {};
    ...
    Resource.useXdr = !!(window['XDomainRequest'] && !('withCredentials' in (new XMLHttpRequest())));
    Resource.tempAnchor = null;
    // some status constants
    Resource.STATUS_NONE = 0;
    Resource.STATUS_OK = 200;
    Resource.STATUS_EMPTY = 204;
    Resource.STATUS_IE_BUG_EMPTY = 1223;
    Resource.STATUS_TYPE_OK = 2;
    ...
    class MiniSignal {
      ...
      * @private
      */
        static _addMiniSignalBinding(self, node) {
            if (!self._head) {
                self._head = node;
                self._tail = node;
            }
