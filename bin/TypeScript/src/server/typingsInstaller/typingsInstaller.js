    ...
    (function (server) {
        ...
        (function (typingsInstaller) {
            ...
            function isPackageOrBowerJson(fileName) {
                const base = ts.getBaseFileName(fileName);
            ...
            class TypingsInstaller {
                    ...
                    if (this.installTypingHost.fileExists(packageJson) && this.installTypingHost.fileExists(packageLockJson)) {
                        ...
                        if (npmConfig.devDependencies && npmLock.dependencies) {
                            for (const key in npmConfig.devDependencies) {
                                ...
                                const semver = ts.Semver.parse(version);
                                const newTyping = { typingLocation: typingFile, version: semver };
                ...
                filterTypings(typingsToInstall) {
                    return typingsToInstall.filter(typing => {
                        if (this.missingTypingsSet.get(typing)) {
                            if (this.log.isEnabled())
                                this.log.writeLine(`'${typing}' is in missingTypingsSet - skipping...`);
                            return false;
                        }
                        ...
                        if (this.packageNameToTypingLocation.get(typing) && ts.JsTyping.isTypingUpToDate(this.packageNameToTypingLocation.get(typing), this.typesRegistry.get(typing))) {
                            if (this.log.isEnabled())
                                this.log.writeLine(`'${typing}' already has an up-to-date typing - skipping...`);
                            return false;
                        }
                        return true;
                    });
                    ...
                    this.installTypingsAsync(requestId, scopedTypings, cachePath, ok => {
                        try {
                            ...
                            for (const packageName of filteredTypings) {
                                ...
                                const newVersion = ts.Semver.parse(distTags[`ts${ts.versionMajorMinor}`] || distTags[latestDistTag]);
                ...
                watchFiles(projectName, files, projectRootPath) {
                    if (!files.length) {
                    ...
                    const createProjectWatcher = (path, createWatch) => {
                    ...
                    const createProjectFileWatcher = (file) => {
                        if (isLoggingEnabled) {
                            this.log.writeLine(`FileWatcher:: Added:: WatchInfo: ${file}`);
                        }
                        const watcher = this.installTypingHost.watchFile(file, (f, eventKind) => {
                            if (isLoggingEnabled) {
                                this.log.writeLine(`FileWatcher:: Triggered with ${f} eventKind: ${ts.FileWatcherEventKind[eventKind]}:: WatchInfo: ${file}:: handler is already invoked '${watchers.isInvoked}'`);
                            }
                    ...
                    const createProjectDirectoryWatcher = (dir) => {
                        if (isLoggingEnabled) {
