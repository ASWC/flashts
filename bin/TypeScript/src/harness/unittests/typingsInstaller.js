...
(function (ts) {
    ...
    (function (projectSystem) {
        ...
        var validatePackageName = ts.JsTyping.validatePackageName;
        class Installer extends projectSystem.TestTypingsInstaller {
            constructor(host, p, log) {
                super((p && p.globalTypingsCacheLocation) || "/a/data", (p && p.throttleLimit) || 5, host, (p && p.typesRegistry), log);
            }
            installAll(expectedCount) {
                this.checkPendingCommands(expectedCount);
                this.executePendingCommands();
            }
        }
        function executeCommand(self, host, installedTypings, typingFiles, cb) {
        ...
        var typingsName = TI.typingsName;
        describe("local module", () => {
            it("should not be picked up", () => {
                ...
                const host = projectSystem.createServerHost([f1, f2, config, typesConfig]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("config"), globalTypingsCacheLocation: typesCache });
                    }
                    installWorker(_requestId, _args, _cwd, _cb) {
                        assert(false, "should not be called");
                    }
                })();
                const service = projectSystem.createProjectService(host, { typingsInstaller: installer });
                service.openClientFile(f1.path);
                service.checkNumberOfProjects({ configuredProjects: 1 });
                projectSystem.checkProjectActualFiles(projectSystem.configuredProjectAt(service, 0), [f1.path, f2.path, config.path]);
        ...
        describe("typingsInstaller", () => {
            it("configured projects (typings installed) 1", () => {
                ...
                const host = projectSystem.createServerHost([file1, tsconfig, packageJson]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
                ...
                const projectService = projectSystem.createProjectService(host, { useSingleInferredProject: true, typingsInstaller: installer });
                projectService.openClientFile(file1.path);
                projectSystem.checkNumberOfProjects(projectService, { configuredProjects: 1 });
                const p = projectSystem.configuredProjectAt(projectService, 0);
                projectSystem.checkProjectActualFiles(p, [file1.path, tsconfig.path]);
                const expectedWatchedFiles = ts.createMap();
                expectedWatchedFiles.set(tsconfig.path, 1); // tsserver
                expectedWatchedFiles.set(projectSystem.libFile.path, 1); // tsserver
                ...
                projectSystem.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
                const expectedWatchedDirectoriesRecursive = ts.createMap();
                ...
                projectSystem.checkWatchedDirectoriesDetailed(host, expectedWatchedDirectoriesRecursive, /*recursive*/ true);
                installer.installAll(/*expectedCount*/ 1);
                projectSystem.checkNumberOfProjects(projectService, { configuredProjects: 1 });
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkProjectActualFiles(p, [file1.path, jquery.path, tsconfig.path]);
            ...
            it("inferred project (typings installed)", () => {
                ...
                const host = projectSystem.createServerHost([file1, packageJson]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
                ...
                const projectService = projectSystem.createProjectService(host, { useSingleInferredProject: true, typingsInstaller: installer });
                projectService.openClientFile(file1.path);
                projectSystem.checkNumberOfProjects(projectService, { inferredProjects: 1 });
                const p = projectService.inferredProjects[0];
                projectSystem.checkProjectActualFiles(p, [file1.path]);
                installer.installAll(/*expectedCount*/ 1);
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkNumberOfProjects(projectService, { inferredProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [file1.path, jquery.path]);
            });
            it("external project - no type acquisition, no .d.ts/js files", () => {
                ...
                const projectService = projectSystem.createProjectService(host, { typingsInstaller: installer });
                projectService.openExternalProject({
                    projectFileName,
                    options: {},
                    rootFiles: [projectSystem.toExternalFile(file1.path)]
                });
            ...
            it("external project - deduplicate from local @types packages", () => {
                ...
                const host = projectSystem.createServerHost([appJs, nodeDts]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("node") });
                    }
                    installWorker() {
                ...
                projectService.openExternalProject({
                    projectFileName,
                    options: {},
                    rootFiles: [projectSystem.toExternalFile(appJs.path)],
            ...
            it("external project - no type acquisition, with only js, jsx, d.ts files", () => {
                ...
                const host = projectSystem.createServerHost([lodashJs, file2Jsx, file3dts, projectSystem.customTypesMap]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("lodash", "react") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                ...
                projectService.openExternalProject({
                    ...
                    rootFiles: [projectSystem.toExternalFile(lodashJs.path), projectSystem.toExternalFile(file2Jsx.path), projectSystem.toExternalFile(file3dts.path)],
                    typeAcquisition: {}
                });
                const p = projectService.externalProjects[0];
                projectService.checkNumberOfProjects({ externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [file2Jsx.path, file3dts.path]);
                installer.installAll(/*expectedCount*/ 1);
                projectSystem.checkNumberOfProjects(projectService, { externalProjects: 1 });
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkNumberOfProjects(projectService, { externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [file2Jsx.path, file3dts.path, lodashDts.path, reactDts.path]);
            });
            it("external project - type acquisition with enable: false", () => {
                ...
                const host = projectSystem.createServerHost([jqueryJs]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
                ...
                projectService.openExternalProject({
                    projectFileName,
                    options: { allowJS: true, moduleResolution: ts.ModuleResolutionKind.NodeJs },
                    rootFiles: [projectSystem.toExternalFile(jqueryJs.path)],
                    typeAcquisition: { enable: false }
                });
                const p = projectService.externalProjects[0];
                projectService.checkNumberOfProjects({ externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [jqueryJs.path]);
                installer.checkPendingCommands(/*expectedCount*/ 0);
            });
            it("external project - no type acquisition, with js & ts files", () => {
                ...
                const host = projectSystem.createServerHost([jqueryJs, file2Ts]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
                ...
                projectService.openExternalProject({
                    projectFileName,
                    options: { allowJS: true, moduleResolution: ts.ModuleResolutionKind.NodeJs },
                    rootFiles: [projectSystem.toExternalFile(jqueryJs.path), projectSystem.toExternalFile(file2Ts.path)],
                    typeAcquisition: {}
                });
                const p = projectService.externalProjects[0];
                projectService.checkNumberOfProjects({ externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [jqueryJs.path, file2Ts.path]);
                installer.checkPendingCommands(/*expectedCount*/ 0);
                projectSystem.checkNumberOfProjects(projectService, { externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [jqueryJs.path, file2Ts.path]);
            });
            it("external project - with type acquisition, with only js, d.ts files", () => {
                ...
                const host = projectSystem.createServerHost([lodashJs, commanderJs, file3dts, packageJson, projectSystem.customTypesMap]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("jquery", "commander", "moment", "express") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                ...
                projectService.openExternalProject({
                    projectFileName,
                    options: { allowJS: true, moduleResolution: ts.ModuleResolutionKind.NodeJs },
                    rootFiles: [projectSystem.toExternalFile(lodashJs.path), projectSystem.toExternalFile(commanderJs.path), projectSystem.toExternalFile(file3dts.path)],
                    typeAcquisition: { enable: true, include: ["jquery", "moment"], exclude: ["lodash"] }
                });
                const p = projectService.externalProjects[0];
                projectService.checkNumberOfProjects({ externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [file3dts.path]);
                installer.installAll(/*expectedCount*/ 1);
                projectSystem.checkNumberOfProjects(projectService, { externalProjects: 1 });
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkNumberOfProjects(projectService, { externalProjects: 1 });
                ...
                projectSystem.checkProjectActualFiles(p, [file3dts.path, commander.path, jquery.path, moment.path, express.path]);
            });
            it("Throttle - delayed typings to install", () => {
                ...
                const host = projectSystem.createServerHost([lodashJs, commanderJs, file3, packageJson, projectSystem.customTypesMap]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { throttleLimit: 3, typesRegistry: projectSystem.createTypesRegistry("commander", "express", "jquery", "moment", "lodash") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                ...
                projectService.openExternalProject({
                    projectFileName,
                    options: { allowJS: true, moduleResolution: ts.ModuleResolutionKind.NodeJs },
                    rootFiles: [projectSystem.toExternalFile(lodashJs.path), projectSystem.toExternalFile(commanderJs.path), projectSystem.toExternalFile(file3.path)],
                    typeAcquisition: { include: ["jquery", "moment"] }
                });
                const p = projectService.externalProjects[0];
                projectService.checkNumberOfProjects({ externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [file3.path]);
                ...
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkNumberOfProjects(projectService, { externalProjects: 1 });
                projectSystem.checkProjectActualFiles(p, [file3.path, commander.path, express.path, jquery.path, moment.path, lodash.path]);
            });
            it("Throttle - delayed run install requests", () => {
                ...
                const host = projectSystem.createServerHost([lodashJs, commanderJs, file3, projectSystem.customTypesMap]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { throttleLimit: 1, typesRegistry: projectSystem.createTypesRegistry("commander", "jquery", "lodash", "cordova", "gulp", "grunt") });
                    }
                    installWorker(_requestId, args, _cwd, cb) {
                        let typingFiles = [];
                ...
                projectService.openExternalProject({
                    ...
                    options: { allowJS: true, moduleResolution: ts.ModuleResolutionKind.NodeJs },
                    rootFiles: [projectSystem.toExternalFile(file3.path)],
                ...
                projectSystem.checkProjectActualFiles(p1, [file3.path]);
                projectSystem.checkProjectActualFiles(p2, [file3.path]);
                installer.executePendingCommands();
                ...
                projectSystem.checkProjectActualFiles(p1, [file3.path, commander.path, jquery.path, lodash.path, cordova.path]);
                projectSystem.checkProjectActualFiles(p2, [file3.path, grunt.path, gulp.path]);
            });
            it("configured projects discover from node_modules", () => {
                ...
                const host = projectSystem.createServerHost([app, jsconfig, jquery, jqueryPackage, nestedPackage]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { globalTypingsCacheLocation: "/tmp", typesRegistry: projectSystem.createTypesRegistry("jquery", "nested") });
                    }
                    installWorker(_requestId, args, _cwd, cb) {
                ...
                projectSystem.checkNumberOfProjects(projectService, { configuredProjects: 1 });
                const p = projectSystem.configuredProjectAt(projectService, 0);
                projectSystem.checkProjectActualFiles(p, [app.path, jsconfig.path]);
                installer.installAll(/*expectedCount*/ 1);
                projectSystem.checkNumberOfProjects(projectService, { configuredProjects: 1 });
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkProjectActualFiles(p, [app.path, jqueryDTS.path, jsconfig.path]);
            });
            it("configured projects discover from bower_components", () => {
                ...
                const host = projectSystem.createServerHost([app, jsconfig, jquery, jqueryPackage]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { globalTypingsCacheLocation: "/tmp", typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                        const installedTypings = ["@types/jquery"];
                        const typingFiles = [jqueryDTS];
                ...
                const p = projectSystem.configuredProjectAt(projectService, 0);
                projectSystem.checkProjectActualFiles(p, [app.path, jsconfig.path]);
                const watchedFilesExpected = ts.createMap();
                watchedFilesExpected.set(jsconfig.path, 1); // project files
                watchedFilesExpected.set(projectSystem.libFile.path, 1); // project files
                projectSystem.checkWatchedFilesDetailed(host, watchedFilesExpected);
                projectSystem.checkWatchedDirectories(host, ts.emptyArray, /*recursive*/ false);
                const watchedRecursiveDirectoriesExpected = ts.createMap();
                watchedRecursiveDirectoriesExpected.set("/", 2); // wild card + type installer
                projectSystem.checkWatchedDirectoriesDetailed(host, watchedRecursiveDirectoriesExpected, /*recursive*/ true);
                installer.installAll(/*expectedCount*/ 1);
                projectSystem.checkNumberOfProjects(projectService, { configuredProjects: 1 });
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkProjectActualFiles(p, [app.path, jqueryDTS.path, jsconfig.path]);
            ...
            it("configured projects discover from bower.json", () => {
                ...
                const host = projectSystem.createServerHost([app, jsconfig, bowerJson]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { globalTypingsCacheLocation: "/tmp", typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
            ...
            it("Malformed package.json should be watched", () => {
                ...
                const host = projectSystem.createServerHost([f, brokenPackageJson]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { globalTypingsCacheLocation: cachePath, typesRegistry: projectSystem.createTypesRegistry("commander") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                ...
                projectSystem.checkProjectActualFiles(service.inferredProjects[0], [f.path, commander.path]);
            });
            it("should install typings for unresolved imports", () => {
                ...
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { globalTypingsCacheLocation: cachePath, typesRegistry: projectSystem.createTypesRegistry("node", "commander") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                ...
                service.checkNumberOfProjects({ inferredProjects: 1 });
                projectSystem.checkProjectActualFiles(service.inferredProjects[0], [file.path]);
                installer.installAll(/*expectedCount*/ 1);
                assert.isTrue(host.fileExists(node.path), "typings for 'node' should be created");
                assert.isTrue(host.fileExists(commander.path), "typings for 'commander' should be created");
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkProjectActualFiles(service.inferredProjects[0], [file.path, node.path, commander.path]);
            });
            it("should pick typing names from non-relative unresolved imports", () => {
                ...
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { globalTypingsCacheLocation: "/tmp", typesRegistry: projectSystem.createTypesRegistry("foo") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                ...
                proj.updateGraph();
                assert.deepEqual(proj.cachedUnresolvedImportsPerFile.get(f1.path), ["foo", "foo", "foo", "@bar/router", "@bar/common", "@bar/common"]);
                installer.installAll(/*expectedCount*/ 1);
            });
            ...
            it("expired cache entry (inferred project, should install typings)", () => {
                ...
                const host = projectSystem.createServerHost([file1, packageJson, jquery, cacheConfig, cacheLockConfig]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
            ...
            it("non-expired cache entry (inferred project, should not install typings)", () => {
                ...
                const host = projectSystem.createServerHost([file1, packageJson, timestamps, cacheConfig, cacheLockConfig, jquery]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { typesRegistry: projectSystem.createTypesRegistry("jquery") });
                    }
        ...
        describe("Validate package name:", () => {
            it("name cannot be too long", () => {
                ...
                assert.equal(validatePackageName(packageName), 3 /* NameTooLong */);
            });
            it("name cannot start with dot", () => {
                assert.equal(validatePackageName(".foo"), 4 /* NameStartsWithDot */);
            });
            it("name cannot start with underscore", () => {
                assert.equal(validatePackageName("_foo"), 5 /* NameStartsWithUnderscore */);
            });
            it("scoped packages not supported", () => {
                assert.equal(validatePackageName("@scope/bar"), 1 /* ScopedPackagesNotSupported */);
            });
            it("non URI safe characters are not supported", () => {
                assert.equal(validatePackageName("  scope  "), 6 /* NameContainsNonURISafeCharacters */);
                assert.equal(validatePackageName("; say ‘Hello from TypeScript!’ #"), 6 /* NameContainsNonURISafeCharacters */);
                assert.equal(validatePackageName("a/b/c"), 6 /* NameContainsNonURISafeCharacters */);
            });
        });
        describe("Invalid package names", () => {
            it("should not be installed", () => {
                ...
                const messages = [];
                const host = projectSystem.createServerHost([f1, packageJson]);
                const installer = new (class extends Installer {
                    constructor() {
        ...
        describe("discover typings", () => {
            ...
            it("should use mappings from safe list", () => {
                ...
                const safeList = ts.createMapFromTemplate({ jquery: "jquery", chroma: "chroma-js" });
                const host = projectSystem.createServerHost([app, jquery, chroma]);
                const logger = trackingLogger();
                const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path, jquery.path, chroma.path], ts.getDirectoryPath(app.path), safeList, ts.emptyMap, { enable: true }, ts.emptyArray, ts.emptyMap);
            ...
            it("should return node for core modules", () => {
                ...
                for (const name of ts.JsTyping.nodeCoreModuleList) {
                    const logger = trackingLogger();
                    const result = ts.JsTyping.discoverTypings(host, logger.log, [f.path], ts.getDirectoryPath(f.path), emptySafeList, cache, { enable: true }, [name, "somename"], ts.emptyMap);
            ...
            it("should use cached locations", () => {
                ...
                const host = projectSystem.createServerHost([f, node]);
                const cache = ts.createMapFromTemplate({ node: { typingLocation: node.path, version: ts.Semver.parse("1.3.0") } });
                const registry = projectSystem.createTypesRegistry("node");
                const logger = trackingLogger();
                const result = ts.JsTyping.discoverTypings(host, logger.log, [f.path], ts.getDirectoryPath(f.path), emptySafeList, cache, { enable: true }, ["fs", "bar"], registry);
                assert.deepEqual(logger.finish(), [
            ...
            it("should search only 2 levels deep", () => {
                ...
                const host = projectSystem.createServerHost([app, a, b]);
                const cache = ts.createMap();
                const logger = trackingLogger();
                const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path], ts.getDirectoryPath(app.path), emptySafeList, cache, { enable: true }, /*unresolvedImports*/ [], ts.emptyMap);
                assert.deepEqual(logger.finish(), [
            ...
            it("should install expired typings", () => {
                ...
                const host = projectSystem.createServerHost([app]);
                const cache = ts.createMapFromTemplate({
                    node: { typingLocation: node.path, version: ts.Semver.parse("1.3.0") },
                    commander: { typingLocation: commander.path, version: ts.Semver.parse("1.0.0") }
                });
                const registry = projectSystem.createTypesRegistry("node", "commander");
                const logger = trackingLogger();
                const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path], ts.getDirectoryPath(app.path), emptySafeList, cache, { enable: true }, ["http", "commander"], registry);
                assert.deepEqual(logger.finish(), [
            ...
            it("should install expired typings with prerelease version of tsserver", () => {
                ...
                const host = projectSystem.createServerHost([app]);
                const cache = ts.createMapFromTemplate({
                    node: { typingLocation: node.path, version: ts.Semver.parse("1.0.0") }
                });
                const registry = projectSystem.createTypesRegistry("node");
                registry.delete(`ts${ts.versionMajorMinor}`);
                const logger = trackingLogger();
                const result = ts.JsTyping.discoverTypings(host, logger.log, [app.path], ts.getDirectoryPath(app.path), emptySafeList, cache, { enable: true }, ["http"], registry);
                assert.deepEqual(logger.finish(), [
            ...
            it("prerelease typings are properly handled", () => {
                ...
                const host = projectSystem.createServerHost([app]);
                const cache = ts.createMapFromTemplate({
                    node: { typingLocation: node.path, version: ts.Semver.parse("1.3.0-next.0") },
                    commander: { typingLocation: commander.path, version: ts.Semver.parse("1.3.0-next.0") }
                });
                const registry = projectSystem.createTypesRegistry("node", "commander");
        ...
        describe("telemetry events", () => {
            it("should be received", () => {
                ...
                const host = projectSystem.createServerHost([f1, packageFile]);
                ...
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkNumberOfProjects(projectService, { inferredProjects: 1 });
                projectSystem.checkProjectActualFiles(projectService.inferredProjects[0], [f1.path, commander.path]);
            });
        });
        describe("progress notifications", () => {
            it("should be sent for success", () => {
                ...
                const host = projectSystem.createServerHost([f1, packageFile, packageLockFile]);
                let beginEvent;
                let endEvent;
                const installer = new (class extends Installer {
                    constructor() {
            ...
            it("should be sent for error", () => {
                ...
                projectSystem.checkNumberOfProjects(projectService, { inferredProjects: 1 });
                projectSystem.checkProjectActualFiles(projectService.inferredProjects[0], [f1.path]);
            });
        });
        ...
        describe("recomputing resolutions of unresolved imports", () => {
            ...
            const appPath = "/a/b/app.js";
            const foooPath = "/a/b/node_modules/fooo/index.d.ts";
            function verifyResolvedModuleOfFooo(project) {
            ...
            function verifyUnresolvedImportResolutions(appContents, typingNames, typingFiles) {
                const app = {
                    path: appPath,
                ...
                const host = projectSystem.createServerHost([app, fooo]);
                const installer = new (class extends Installer {
                    constructor() {
                        super(host, { globalTypingsCacheLocation, typesRegistry: projectSystem.createTypesRegistry("foo") });
                    }
                    installWorker(_requestId, _args, _cwd, cb) {
                ...
                projectService.checkNumberOfProjects({ inferredProjects: 1 });
                const proj = projectService.inferredProjects[0];
                projectSystem.checkProjectActualFiles(proj, [app.path, fooo.path]);
                const foooResolution1 = verifyResolvedModuleOfFooo(proj);
                installer.installAll(/*expectedCount*/ 1);
                host.checkTimeoutQueueLengthAndRun(2);
                projectSystem.checkProjectActualFiles(proj, typingFiles.map(f => f.path).concat(app.path, fooo.path));
