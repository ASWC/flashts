...
(function (ts) {
    const newLine = "\r\n";
    class SourceText {
        constructor(references, importsAndExports, program, changedPart = 0, version = 0) {
            this.references = references;
            this.importsAndExports = importsAndExports;
            this.program = program;
            this.changedPart = changedPart;
            this.version = version;
        }
        static New(references, importsAndExports, program) {
        ...
        updateReferences(newReferences) {
            ...
            return new SourceText(newReferences + newLine, this.importsAndExports, this.program, this.changedPart | 1 /* references */, this.version + 1);
        }
        updateImportsAndExports(newImportsAndExports) {
            ts.Debug.assert(newImportsAndExports !== undefined);
            return new SourceText(this.references, newImportsAndExports + newLine, this.program, this.changedPart | 2 /* importsAndExports */, this.version + 1);
        }
        updateProgram(newProgram) {
            ts.Debug.assert(newProgram !== undefined);
            return new SourceText(this.references, this.importsAndExports, newProgram, this.changedPart | 4 /* program */, this.version + 1);
        }
        getFullText() {
        ...
        getChangeRange(oldSnapshot) {
            ...
            switch (oldText.changedPart ^ this.changedPart) {
                ...
                case 4 /* program */:
                    ...
                    break;
                default:
                    ts.Debug.assert(false, "Unexpected change");
            }
            return ts.createTextChangeRange(oldSpan, newLength);
        }
    }
    ts.SourceText = SourceText;
    function createSourceFileWithText(fileName, sourceText, target) {
        const file = ts.createSourceFile(fileName, sourceText.getFullText(), target);
        file.sourceText = sourceText;
        file.version = "" + sourceText.getVersion();
        return file;
    }
    function createTestCompilerHost(texts, target, oldProgram) {
        const files = ts.arrayToMap(texts, t => t.name, t => {
            if (oldProgram) {
                let oldFile = oldProgram.getSourceFile(t.name);
        ...
        return {
            ...
            getCanonicalFileName(fileName) {
                return ts.sys && ts.sys.useCaseSensitiveFileNames ? fileName : fileName.toLowerCase();
            },
            useCaseSensitiveFileNames() {
                return ts.sys && ts.sys.useCaseSensitiveFileNames;
            },
            getNewLine() {
                return ts.sys ? ts.sys.newLine : newLine;
            },
    ...
    ts.createTestCompilerHost = createTestCompilerHost;
    function newProgram(texts, rootNames, options) {
    ...
    ts.newProgram = newProgram;
    function updateProgram(oldProgram, rootNames, options, updater, newTexts) {
        if (!newTexts) {
    ...
    ts.updateProgram = updateProgram;
    function updateProgramText(files, fileName, newProgramText) {
        const file = ts.find(files, f => f.name === fileName);
        file.text = file.text.updateProgram(newProgramText);
    }
    ts.updateProgramText = updateProgramText;
    function checkResolvedTypeDirective(expected, actual) {
        if (!expected === !actual) {
            if (expected) {
    ...
    function checkCache(caption, program, fileName, expectedContent, getCache, entryChecker) {
    ...
    function mapsAreEqual(left, right, valuesAreEqual) {
        if (left === right)
            return true;
        if (!left || !right)
            return false;
        ...
        if (someInLeftHasNoMatch)
            return false;
        const someInRightHasNoMatch = ts.forEachKey(right, rightKey => !left.has(rightKey));
        return !someInRightHasNoMatch;
    }
    function checkResolvedModulesCache(program, fileName, expectedContent) {
        checkCache("resolved modules", program, fileName, expectedContent, f => f.resolvedModules, ts.checkResolvedModule);
    }
    function checkResolvedTypeDirectivesCache(program, fileName, expectedContent) {
...
`, "", `var x = 1`)
        ...
        it("successful if change does not affect imports", () => {
            ...
            assert.equal(program1.structureIsReused, 2 /* Completely */);
        ...
        it("fails if change affects tripleslash references", () => {
            ...
            assert.equal(program1.structureIsReused, 1 /* SafeModules */);
        });
        it("fails if change affects type references", () => {
            const program1 = newProgram(files, ["a.ts"], { types: ["a"] });
            updateProgram(program1, ["a.ts"], { types: ["b"] }, ts.noop);
            assert.equal(program1.structureIsReused, 0 /* Not */);
        });
        it("succeeds if change doesn't affect type references", () => {
            const program1 = newProgram(files, ["a.ts"], { types: ["a"] });
            updateProgram(program1, ["a.ts"], { types: ["a"] }, ts.noop);
            assert.equal(program1.structureIsReused, 2 /* Completely */);
        });
...
/// <reference types="typerefs1" />`;
        ...
        it("fails if module kind changes", () => {
            ...
            updateProgram(program1, ["a.ts"], { target, module: ts.ModuleKind.AMD }, ts.noop);
            assert.equal(program1.structureIsReused, 0 /* Not */);
        });
        it("fails if rootdir changes", () => {
            const program1 = newProgram(files, ["a.ts"], { target, module: ts.ModuleKind.CommonJS, rootDir: "/a/b" });
            updateProgram(program1, ["a.ts"], { target, module: ts.ModuleKind.CommonJS, rootDir: "/a/c" }, ts.noop);
            assert.equal(program1.structureIsReused, 0 /* Not */);
        });
        it("fails if config path changes", () => {
            const program1 = newProgram(files, ["a.ts"], { target, module: ts.ModuleKind.CommonJS, configFilePath: "/a/b/tsconfig.json" });
            updateProgram(program1, ["a.ts"], { target, module: ts.ModuleKind.CommonJS, configFilePath: "/a/c/tsconfig.json" }, ts.noop);
            assert.equal(program1.structureIsReused, 0 /* Not */);
        });
        it("succeeds if missing files remain missing", () => {
            const options = { target, noLib: true };
            ...
            assert.equal(2 /* Completely */, program1.structureIsReused);
        });
        it("fails if missing file is created", () => {
            const options = { target, noLib: true };
            const program1 = newProgram(files, ["a.ts"], options);
            assert.notDeepEqual(ts.emptyArray, program1.getMissingFilePaths());
            const newTexts = files.concat([{ name: "non-existing-file.ts", text: SourceText.New("", "", `var x = 1`) }]);
            const program2 = updateProgram(program1, ["a.ts"], options, ts.noop, newTexts);
            assert.lengthOf(program2.getMissingFilePaths(), 0);
            assert.equal(0 /* Not */, program1.structureIsReused);
        });
        it("resolution cache follows imports", () => {
            ...
            const program1 = newProgram(files, ["a.ts"], options);
            checkResolvedModulesCache(program1, "a.ts", ts.createMapFromTemplate({ b: ts.createResolvedModule("b.ts") }));
            ...
            assert.equal(program2.structureIsReused, 1 /* SafeModules */);
            ...
            assert.equal(program3.structureIsReused, 1 /* SafeModules */);
            checkResolvedModulesCache(program4, "a.ts", ts.createMapFromTemplate({ b: ts.createResolvedModule("b.ts"), c: undefined }));
        });
        it("set the resolvedImports after re-using an ambient external module declaration", () => {
            ...
            const options = { target, typeRoots: ["/types"] };
            const program1 = newProgram(files, ["/a.ts"], options);
            const program2 = updateProgram(program1, ["/a.ts"], options, files => {
        ...
        it("resolved type directives cache follows type directives", () => {
            ...
            const options = { target, typeRoots: ["/types"] };
            const program1 = newProgram(files, ["/a.ts"], options);
            checkResolvedTypeDirectivesCache(program1, "/a.ts", ts.createMapFromTemplate({ typedefs: { resolvedFileName: "/types/typedefs/index.d.ts", primary: true } }));
        ...
        it("fetches imports after npm install", () => {
            ...
            const options = { target: ts.ScriptTarget.ES2015, traceResolution: true, moduleResolution: ts.ModuleResolutionKind.NodeJs };
        ...
        it("can reuse module resolutions from non-modified files", () => {
            ...
            const options = { target: ts.ScriptTarget.ES2015, traceResolution: true, moduleResolution: ts.ModuleResolutionKind.Classic };
        ...
        describe("redirects", () => {
            ...
            function createRedirectProgram(options) {
                const files = [
                    {
            ...
            function updateRedirectProgram(program, updater) {
    ...
    var createTestSystem = ts.TestFSWithWatch.createWatchedSystem;
    ...
    describe("isProgramUptoDate should return true when there is no change in compiler options and", () => {
        function verifyProgramIsUptoDate(program, newRootFileNames, newOptions) {
            const actual = ts.isProgramUptoDate(program, newRootFileNames, newOptions, path => program.getSourceFileByPath(path).version, /*fileExists*/ ts.returnFalse, 
            /*hasInvalidatedResolution*/ ts.returnFalse, 
            /*hasChangedAutomaticTypeDirectiveNames*/ false);
            assert.isTrue(actual);
        }
        function duplicate(filesOrOptions) {
            return JSON.parse(JSON.stringify(filesOrOptions));
        }
        function verifyProgramWithoutConfigFile(system, rootFiles, options) {
            const program = ts.createWatchProgram(ts.createWatchCompilerHostOfFilesAndCompilerOptions(rootFiles, options, system)).getCurrentProgram().getProgram();
            verifyProgramIsUptoDate(program, duplicate(rootFiles), duplicate(options));
        }
        function verifyProgramWithConfigFile(system, configFileName) {
            const program = ts.createWatchProgram(ts.createWatchCompilerHostOfConfigFile(configFileName, {}, system)).getCurrentProgram().getProgram();
            const { fileNames, options } = ts.parseConfigFileWithSystem(configFileName, {}, system, ts.notImplemented);
            verifyProgramIsUptoDate(program, fileNames, options);
        }
        function verifyProgram(files, rootFiles, options, configFile) {
        ...
        it("has lib specified in the options", () => {
            const compilerOptions = { lib: ["es5", "es2015.promise"] };
            const app = {
                path: "/src/app.ts",
