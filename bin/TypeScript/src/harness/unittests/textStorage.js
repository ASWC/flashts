...
(function (ts) {
    ...
    (function (textStorage) {
        describe("Text storage", () => {
            ...
            it("text based storage should be have exactly the same as script version cache", () => {
                ...
                const ts1 = new ts.server.TextStorage(host, ts.server.asNormalizedPath(f.path));
                const ts2 = new ts.server.TextStorage(host, ts.server.asNormalizedPath(f.path));
    ...
    })(textStorage = ts.textStorage || (ts.textStorage = {}));
})(ts || (ts = {}));
