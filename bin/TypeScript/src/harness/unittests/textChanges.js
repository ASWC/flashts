...
(function (ts) {
    describe("textChanges", () => {
        ...
        function getRuleProvider(placeOpenBraceOnNewLineForFunctions) {
            return ts.formatting.getFormatContext(placeOpenBraceOnNewLineForFunctions ? Object.assign({}, ts.testFormatOptions, { placeOpenBraceOnNewLineForFunctions: true }) : ts.testFormatOptions);
        }
        ...
        function verifyPositions(node, text) {
        ...
        function runSingleFileTest(caption, placeOpenBraceOnNewLineForFunctions, text, validateNodes, testBlock) {
            it(caption, () => {
    ...
    namespace M2
    {
        function foo() {
            ...
            runSingleFileTest("extractMethodLike", /*placeOpenBraceOnNewLineForFunctions*/ true, text, /*validateNodes*/ true, (sourceFile, changeTracker) => {
                const statements = findChild("foo", sourceFile).body.statements.slice(1);
                ...
                changeTracker.replaceNodeRange(sourceFile, statements[0], ts.lastOrUndefined(statements), newStatement, { suffix: newLineCharacter });
            });
        }
        {
            const text = `
...
`;
        ...
        function findVariableStatementContaining(name, sourceFile) {
            return ts.cast(findVariableDeclarationContaining(name, sourceFile).parent.parent, ts.isVariableStatement);
        }
        function findVariableDeclarationContaining(name, sourceFile) {
...
`;
            ...
            runSingleFileTest("deleteNodeRange2", /*placeOpenBraceOnNewLineForFunctions*/ false, text, /*validateNodes*/ false, (sourceFile, changeTracker) => {
                changeTracker.deleteNodeRange(sourceFile, findVariableStatementContaining("y", sourceFile), findVariableStatementContaining("z", sourceFile), { useNonAdjustedStartPosition: true });
            });
            runSingleFileTest("deleteNodeRange3", /*placeOpenBraceOnNewLineForFunctions*/ false, text, /*validateNodes*/ false, (sourceFile, changeTracker) => {
                changeTracker.deleteNodeRange(sourceFile, findVariableStatementContaining("y", sourceFile), findVariableStatementContaining("z", sourceFile), { useNonAdjustedEndPosition: true });
            });
            runSingleFileTest("deleteNodeRange4", /*placeOpenBraceOnNewLineForFunctions*/ false, text, /*validateNodes*/ false, (sourceFile, changeTracker) => {
                changeTracker.deleteNodeRange(sourceFile, findVariableStatementContaining("y", sourceFile), findVariableStatementContaining("z", sourceFile), { useNonAdjustedStartPosition: true, useNonAdjustedEndPosition: true });
            });
        }
        function createTestVariableDeclaration(name) {
...
namespace M {
    ...
    var a = 4; // comment 7
        ...
        function findConstructor(sourceFile) {
            const classDecl = sourceFile.statements[0];
            return ts.find(classDecl.members, (m) => ts.isConstructorDeclaration(m) && !!m.body);
        }
