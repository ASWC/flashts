...
(function (ts) {
    describe("Transpile", () => {
        function transpilesCorrectly(name, input, testSettings) {
            describe(name, () => {
                let justName;
                let transpileOptions;
                let canUseOldTranspile;
                let toBeCompiled;
                let transpileResult;
                let oldTranspileResult;
                let oldTranspileDiagnostics;
                ...
                justName = "transpile/" + name.replace(/[^a-z0-9\-. ]/ig, "") + (transpileOptions.compilerOptions.jsx ? ts.Extension.Tsx : ts.Extension.Ts);
                toBeCompiled = [{
...
var x = 0;`, {
        ...
        transpilesCorrectly("Report an error when compiler-options module-kind is out-of-range", "", {
            options: { compilerOptions: { module: 123 } }
        });
