...
var ts;
(function (ts) {
    const testContentsJson = ts.createMapFromTemplate({
        "/dev/tsconfig.json": {
    ...
    const testContents = ts.mapEntries(testContentsJson, (k, v) => [k, ts.isString(v) ? v : JSON.stringify(v)]);
    const caseInsensitiveBasePath = "c:/dev/";
    const caseInsensitiveHost = new Utils.MockParseConfigHost(caseInsensitiveBasePath, /*useCaseSensitiveFileNames*/ false, ts.mapEntries(testContents, (key, content) => [`c:${key}`, content]));
    ...
    describe("configurationExtension", () => {
        ts.forEach([
            ...
            function getParseCommandLine(entry) {
            ...
            function getParseCommandLineJsonSourceFile(entry) {
            ...
            function testSuccess(name, entry, expected, expectedFiles) {
            ...
            function testFailure(name, entry, expectedDiagnostics) {
                it(name, () => {
                    const parsed = getParseCommandLine(entry);
