...
describe("Colorization", () => {
    ...
    function getEntryAtPosition(result, position) {
        let entryPosition = 0;
    ...
    function punctuation(text, position) { return createClassification(text, ts.TokenClass.Punctuation, position); }
    function keyword(text, position) { return createClassification(text, ts.TokenClass.Keyword, position); }
    function operator(text, position) { return createClassification(text, ts.TokenClass.Operator, position); }
    function comment(text, position) { return createClassification(text, ts.TokenClass.Comment, position); }
    function whitespace(text, position) { return createClassification(text, ts.TokenClass.Whitespace, position); }
    function identifier(text, position) { return createClassification(text, ts.TokenClass.Identifier, position); }
    function numberLiteral(text, position) { return createClassification(text, ts.TokenClass.NumberLiteral, position); }
    function stringLiteral(text, position) { return createClassification(text, ts.TokenClass.StringLiteral, position); }
    function finalEndOfLineState(value) { return { value, classification: undefined, position: 0 }; }
    function createClassification(value, classification, position) {
        return { value, classification, position };
    }
    function testLexicalClassification(text, initialEndOfLineState, ...expectedEntries) {
    ...
    describe("test getClassifications", () => {
        it("Returns correct token classes", () => {
            testLexicalClassification("var x: string = \"foo\"; //Hello", ts.EndOfLineState.None, keyword("var"), whitespace(" "), identifier("x"), punctuation(":"), keyword("string"), operator("="), stringLiteral("\"foo\""), comment("//Hello"), punctuation(";"));
        });
