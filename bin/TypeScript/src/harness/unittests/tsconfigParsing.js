...
(function (ts) {
    describe("parseConfigFileTextToJson", () => {
        function assertParseResult(jsonText, expectedConfigObject) {
        ...
        function assertParseError(jsonText) {
            const parsed = ts.parseConfigFileTextToJson("/apath/tsconfig.json", jsonText);
            assert.deepEqual(parsed.config, {});
            assert.isTrue(undefined !== parsed.error);
        }
        function assertParseErrorWithExcludesKeyword(jsonText) {
            {
                const parsed = ts.parseConfigFileTextToJson("/apath/tsconfig.json", jsonText);
        ...
        function getParsedCommandJson(jsonText, configFileName, basePath, allFileList) {
            const parsed = ts.parseConfigFileTextToJson(configFileName, jsonText);
            const host = new Utils.MockParseConfigHost(basePath, true, allFileList);
            return ts.parseJsonConfigFileContent(parsed.config, host, basePath, /*existingOptions*/ undefined, configFileName);
        }
        function getParsedCommandJsonNode(jsonText, configFileName, basePath, allFileList) {
        ...
        function assertParseFileList(jsonText, configFileName, basePath, allFileList, expectedFileList) {
            {
        ...
        function assertParseFileDiagnostics(jsonText, configFileName, basePath, allFileList, expectedDiagnosticCode, noLocation) {
            {
                const parsed = getParsedCommandJson(jsonText, configFileName, basePath, allFileList);
