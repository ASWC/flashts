if (typeof describe === "undefined") {
    global.describe = undefined; // If launched without mocha for parallel mode, we still need a global describe visible to satisfy the parsing of the unit tests
    global.it = undefined;
}
var Harness;
(function (Harness) {
    ...
    (function (Parallel) {
        ...
        (function (Host) {
            ...
            function perfdataFileName(target) {
                return `${perfdataFileNameFragment}${target ? `.${target}` : ""}.json`;
            }
            function readSavedPerfData(target) {
            ...
            Host.start = start;
            function startDelayed(perfData, totalCost) {
                ...
                for (let i = 0; i < workerCount; i++) {
                    ...
                    const config = { light: Harness.lightMode, listenForWork: true, runUnitTests, stackTraceLimit };
        ...
        Stack: ${data.payload.stack}`);
                ...
                function updateProgress(percentComplete, title, titleColor) {
                ...
                global.describe = ts.noop; // Disable unit tests
                return;
            }
            let mocha;
            let base;
            ...
            class ProgressBars {
                ...
                update(index, percentComplete, color, title, titleColor) {
        ...
        })(Host = Parallel.Host || (Parallel.Host = {}));
