    ...
    (function (SymbolDisplay) {
        ...
        function getSymbolKindOfConstructorPropertyMethodAccessorFunctionOrVar(typeChecker, symbol, location) {
            ...
            if (flags & ts.SymbolFlags.Property) {
                if (flags & ts.SymbolFlags.Transient && symbol.checkFlags & 6 /* Synthetic */) {
        ...
        function getSymbolDisplayPartsDocumentationAndSymbolKind(typeChecker, symbol, sourceFile, enclosingDeclaration, location, semanticMeaning = ts.getMeaningFromLocation(location), alias) {
            const displayParts = [];
            let documentation;
            let tags;
            ...
            let hasAddedSymbolInfo;
            const isThisExpression = location.kind === ts.SyntaxKind.ThisKeyword && ts.isExpression(location);
            let type;
            let printer;
            ...
            if (symbolKind !== ts.ScriptElementKind.unknown || symbolFlags & ts.SymbolFlags.Class || symbolFlags & ts.SymbolFlags.Alias) {
                ...
                if (callExpressionLike) {
                    ...
                    const locationIsSymbolDeclaration = ts.find(symbol.declarations, declaration => declaration === (location.kind === ts.SyntaxKind.ConstructorKeyword ? functionDeclaration.parent : functionDeclaration));
            ...
            if (symbolFlags & ts.SymbolFlags.Module) {
                prefixNextMeaning();
                const declaration = ts.getDeclarationOfKind(symbol, ts.SyntaxKind.ModuleDeclaration);
                ...
                if (symbol.parent) {
                    ...
                    if (declaration) {
                        if (ts.isFunctionLikeKind(declaration.kind)) {
                            ...
                            else if (declaration.kind !== ts.SyntaxKind.CallSignature && declaration.name) {
            ...
            if (symbolFlags & ts.SymbolFlags.Alias) {
                ...
                switch (symbol.declarations[0].kind) {
                    ...
                    case ts.SyntaxKind.ExportAssignment:
                        displayParts.push(ts.keywordPart(ts.SyntaxKind.ExportKeyword));
                        displayParts.push(ts.spacePart());
                        displayParts.push(ts.keywordPart(symbol.declarations[0].isExportEquals ? ts.SyntaxKind.EqualsToken : ts.SyntaxKind.DefaultKeyword));
                        break;
                        ...
                        if (symbolKind === ts.ScriptElementKind.memberVariableElement ||
                            ...
                            if (type.symbol && type.symbol.flags & ts.SymbolFlags.TypeParameter) {
                                const typeParameterParts = ts.mapToDisplayParts(writer => {
                                    const param = typeChecker.typeParameterToDeclaration(type, enclosingDeclaration);
